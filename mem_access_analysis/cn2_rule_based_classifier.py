...
class CN2:
    ...
    def predict(self, test_data, rule_list):
        """
    ...
    def compute_selectors(self):
        ...
        """
        #print("Inside compute_selectors")
    ...
    def find_best_complex(self):
        ...
        '''
        #print("Inside find_best_complex")
        best_complex = None
    ...
    def get_covered_examples(self, all_examples, best_cpx):
        ...
        # best complex.
        #print("Inside get_covered_examples")
        values = dict()
    ...
    def get_most_common_class(self, covered_examples):
        ...
        '''
        #print("Inside the get_most_common_class")
        ...
        '''
        classes = self.data.loc[covered_examples, [list(self.data)[-1]]]  # last column is the class
        vc = classes.value_counts()
        count = vc.max()
        most_common_class = vc.idxmax()
        return most_common_class, count
        '''
        col = list(self.data)[-1]
        classes = self.data.loc[covered_examples, col]
    ...
    def specialize_star(self, star, selectors):
        ...
        '''
        #print("Inside the specialize_star")
        new_star = []
        if len(star) > 0:
            for complex in star:
...
if __name__ == "__main__":
    ...
    print('------------------------------')
    print('------------------------------')
    print('ba_merged.csv DATASET')
    print('performing the equal frequency binning')
    cn2 = CN2()
    train_start = time.time()
    df = pd.read_csv("../mem_access_traces/std_merged.csv")

    num_cols = df.select_dtypes(include=['float64', 'int64']).columns
    num_cols = [c for c in num_cols if c != "Fine_grained_Target"]

    for col in num_cols:
        df[col] = pd.qcut(df[col], q=5, duplicates='drop').astype(str)
    
    df_shuffled = df.sample(frac=1, random_state=42).reset_index(drop=True)

    train_rows = int(len(df_shuffled) * 0.8)

    df_train = df_shuffled.iloc[:train_rows].reset_index(drop=True)
    df_test  = df_shuffled.iloc[train_rows:].reset_index(drop=True)
    rules = cn2.fit(df_train)
    ...
    with open('cn2_std_merged.txt', 'wb') as f:
        pickle.dump(rules, f)
    ...
    rules_performance, accuracy = cn2.predict(df_test, rules)
    ...
    with pd.option_context('display.max_rows', None, 'display.max_columns', None):
        print(table)
    table.to_csv('std_performance.csv')
